...
class GraphSLAM():
    ...
    def solve(self):
        # Add infinity to Information matrix where landmark is not observed
...
if __name__ == "__main__":
    # Dataset
    dataset = "GraphSlamV2/data/dataset1"
    start_frame = 50
    end_frame = 3000
    N_iterations = 1
    # State covariance matrix
    R = np.diagflat(np.array([1, 1, 1])) ** 2
    # Measurement covariance matrix
    Q = np.diagflat(np.array([1.0, 1.0, 1.0])) ** 2
