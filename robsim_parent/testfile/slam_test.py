...
route_reduction = 0.01

# ERROR CONFIG
# error_landmark_range = True
# error_landmark_visibility = True
error_landmark_detection = True

# error_gis_recall = True
# error_gis_position = True
...
for error_landmark_range in [False, True]:
    for error_landmark_visibility in [False, True]:
        for error_gis_recall in [False, True]:
            for error_gis_position in [False, True]:

                if some_id_for_plots != 15 and some_id_for_plots != 0:
                    some_id_for_plots += 1
                    continue
		...
		# Putting it all together
                ...
                route = rs.reduce_list(full_route, route_reduction)
                ...
                for pose in tqdm(route):
                    ...
                    for i, landmark in enumerate(ws.landmarks):
                        ...
                        if not any([d > rs.dist_l2(origin, i) for i in intersections]):
                ...
                if include_gis:
                    for i, landmark in enumerate(ws.landmarks):
                        ...
                        if error_gis_recall and random.random() > landmark.type.recall:
                            continue

                        if error_gis_position:
                            noisy_landmark = rs.add_normal_noise_point(landmark, landmark.type.error)
                            gis_points.append((i, noisy_landmark))

                        else:
                            gis_points.append((i, landmark))
                else:
                ...
                if include_gis and plot:
                    ...
                    for i, pt in gis_points:
                        origin = ws.landmarks[i]

                        plt.plot((origin.x, pt.x), (origin.y, pt.y), '-', color='red')

                    plt.plot([pt.x for pt in ws.landmarks], [pt.y for pt in ws.landmarks], 'o', color='blue', markersize=3)
                ...
                slam = rs.Slam(route[0], len(ws.landmarks), var_odometry_d=odometry_std_d * 5, var_odometry_theta1=odometry_std_a1 * 5, var_odometry_theta2=odometry_std_a2 * 5, var_landmarks_d=landmark_std_d * 5, var_landmarks_theta=landmark_std_a * 5, var_gps=std_gps**2, var_gis=0.48**2, landmark_estimates=gis_points)
                ...
                error_landmarks = [b - a for a, b, i in zip(slam.landmarks, ws.landmarks, slam.landmark_initialized) if i]
                ...
                noisy_landmark_diff = [a - b for a, b in zip(ws.landmarks, average_noisy_landmarks)]
                ...
                slam_landmark_diff = [a - b for a, b in zip(ws.landmarks, slam.landmarks)]
                ...
                with open(f'plots/{some_id_for_plots}_results.json', 'w') as f:
                    json.dump({'error_landmark_range': error_landmark_range, 'error_landmark_visibility': error_landmark_visibility, 'error_gis_recall': error_gis_recall, 'error_gis_position': error_gis_position, 'ftol': ftol, 'gis': include_gis, 'gps': n_gps, 'osd': slam_metric_d, 'osa': slam_metric_a, 'ls': slam_landmark_metric}, f)
