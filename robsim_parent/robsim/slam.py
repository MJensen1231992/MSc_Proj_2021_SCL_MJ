...
class Slam:
    ...
    def __init__(self, origin, n_landmarks, var_odometry_d=1, var_odometry_theta1=1, var_odometry_theta2=1, var_landmarks_d=1, var_landmarks_theta=1, var_gps=1, var_gis=1, landmark_estimates=[]):
        self.origin = origin
    ...
    def plot(self, plot_landmark_measurements: bool = False, width=60, height=30, border=1, figsize=(8, 8), dpi=200):
        ...
        for landmark, measurement, _ in self.gis_constraints:
            plt.plot(
                (measurement.x, self.landmarks[landmark].x), 
                (measurement.y, self.landmarks[landmark].y), 
                '-', color='green', linewidth=0.5)

            plt.plot(measurement.x, measurement.y, 'x', markersize=3, color='green')

        plt.plot([p.x for p in self.route], [p.y for p in self.route], '-o', markersize=3, linewidth=1)

        for pose, measurement, _ in self.gps_constraints:
            plt.plot(
                (measurement.x, self.route[pose].x), 
                (measurement.y, self.route[pose].y), 
                '-', color='purple', linewidth=0.5)

            plt.plot(measurement.x, measurement.y, 'x', markersize=3, color='purple')
        
        plt.plot([x_min, x_min,x_max, x_max, x_min], [y_min, y_max, y_max, y_min, y_min], '-', color='black')
