...
from robsim.landmark import Landmark
...
class Workspace:
    def __init__(self, config_file: str, background_file: str):
        ...
        self.landmarks = [Landmark(x, y, name) for x, y, name in ws.setdefault('landmarks', [])]
    ...
    def plot(self, border: int = 1, border_color = 'black', show = False, figname: str = None, grid_size: float = None, figsize: tuple = (12,12), dpi: int = 200):
        ...
        types = set([landmark.type for landmark in self.landmarks])

        for type in types:
            landmarks = [landmark for landmark in self.landmarks if landmark.type.name == type.name]

            xs = [landmark.x for landmark in landmarks]
            ys = [landmark.y for landmark in landmarks]

            plt.plot(xs, ys, 's', c=type.color, markersize=3, label=f'{type.description} - {type.source}')
