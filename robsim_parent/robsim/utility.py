...
def add_normal_noise_point(point, std: float = 0.01):
    offset = Point(np.random.normal(0, std), np.random.normal(0, std))
    return point + offset
...
def to_points(l):
    ...
    if len(l) == 2 and isinstance(l[0], float) and isinstance(l[1], float):
        try:
            point = Point(*l)
            return point
        except Exception:
    ...
    return [to_points(e) for e in l]

def to_poses(l):
    if isinstance(l, int):
        return l

    if len(l) == 3 and isinstance(l[0], float) and isinstance(l[1], float) and isinstance(l[2], float):
        try:
            pose = Pose(*l)
            return pose
        except Exception:
...
def from_points(l):
    ...
    return [from_points(e) for e in l]
...
def from_pose(l):
    ...
    try:
        return [l.x, l.y, l.theta]
    except Exception:
        pass
    
    return [from_points(e) for e in l]

def save_to_json(l, name, indent=4):
